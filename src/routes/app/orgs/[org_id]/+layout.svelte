<script lang="ts">
	...
	$: on_org_page = $page.url.pathname.endsWith(
		`/app/orgs/${$organization?.id}`
	);
	$: on_org_app_page = $page.url.pathname.startsWith(
		`/app/orgs/${$organization?.id}/apps/`
	);
</script>
			...
			{#if on_org_page}
				<div class="flex w-full flex-col gap-2 sm:flex-row sm:gap-4">
					...
					<div>
						<span>
							<a
								class="font-display break-all text-xl font-bold"
								href="/app/orgs/{$organization.id}"
								style:--org="org-name-{$organization.id}"
								>{$organization.name}</a
							>
							<p>
								Owned by {$organization.members[0].user.username ??
									$organization.members[0].user.id}
							</p>
						</span>
					</div>
				</div>
