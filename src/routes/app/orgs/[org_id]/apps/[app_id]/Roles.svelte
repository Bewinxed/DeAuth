...
>
	<div class="flex flex-col place-content-start place-items-start gap-2">
		...
		<p class="text-sm italic">
			By default, all users are created with the role of <span class="badge">USER</span>. You can
			add more roles here and it will be provided to your application when the user authenticates
		</p>
	</div>
...
<div class="p-4">
	{#if $application.app_role.length === 0}
		<EmptyState icon="carbon:user-role" item_name="role">
			<form class="form-control gap-4">
				<label class="label" for="name">
					<span class="label-text capitalize">name</span>
				</label>
				<input type="url" name="name" placeholder="" class="input input-bordered" />
				<button
			...
			{#each $application.app_role as app_role (app_role.id)}
				...
				>
					<span class="inline-flex place-items-center gap-2">
						<Icon icon="carbon:link" class="mr-1 inline" />
						{app_role.name}
					</span>
					<!-- created at -->
					<div class="card">
						<div class="flex place-content-center place-items-center gap-2">
							...
							<div class="card-actions">
								...
								<ModalButton
									...
									item_name="role"
								>
									<Icon icon="carbon:edit" class=" inline" />
										...
										<form
											...
											on:submit|preventDefault={(e) => {
												...
												svetch
													...
													.then((res) => {
														if (res.data) {
															$application.app_role = $application.app_role.map((r) => {
																if (r.id === app_role.id) {
