<script lang="ts">
	...
	$: if (account_id && account_id.length > 3)
		{
			svetch
				.get('app/orgs/:org_id/apps/:app_id/members', {
					path: {
...
>
	...
	<div class="form-control">
		...
		{#key account_id}
			{#if account_id && account_id.length > 3}
				<ul transition:slide
					class="menu menu-md dropdown dropdown-open rounded-box z-[1] w-52 gap-1 border bg-base-100 p-2 shadow"
				>
					{#if $member_search_results.length === 0}
						...
						{#each $member_search_results as member (member.id)}
							<li>
								<button
									class="flex place-content-start place-items-center gap-2"
									on:click|preventDefault="{() => {
										[provider, account_id] = member.user.key[0].id.split(':');
								...
								>
									...
									/>
									<span class="capitalize">{member.user.username ?? trim_id(member.id)}</span>
								</button>
							</li>
						{/each}
					{/if}
	...
	{@const is_found = $member_search_results.find((member) => member.user.key[0].id === key_id)}
