...
>
	<figure
		class="w-20 max-w-sm rotate-12 rounded-full bg-base-300 p-6 text-base-100 shadow-[2px_1px_1px_1px_rgba(204,251,241,0.5)] lg:w-40 hover:bg-primary hover:-translate-y-2 transition-all"
	>
		<Icon {icon} class="h-full w-full"></Icon>
	</figure>
	<h1
		class="flex place-content-center place-items-center text-center text-lg font-semibold text-base-content"
	>
		{caption ? caption : `You have no ${item_name}(s) yet, Create one to get started`}
	</h1>
	{#if $$slots.default}
		<button
		...
		>
			<Icon icon="carbon:add" class="h-4 w-4"></Icon>
			<span class="capitalize">Create {item_name}</span>
		</button>
	{/if}
</div>
<Modal {item_name} {icon} bind:open
	><slot />
	{#if promise}
